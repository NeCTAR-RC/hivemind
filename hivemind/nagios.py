...
COMMIT = 2
SUCCESS = (
    "Your command request was successfully"
...
def host_maintenance(
    comment, start_time=None, hours=2, minutes=0, fixed=False
):
    if not env.nagios:
    ...
    resp = requests.post(
        ...
        data={
            "cmd_typ": CMD_SCHEDULE_HOST_SVC_DOWNTIME,
            "cmd_mod": COMMIT,
...
def cancel_service_maintenance(downtime_id):
    resp = requests.post(
        env.nagios + "cmd.cgi",
        auth=env.nagios_auth,
        data={"cmd_typ": 79, "cmd_mod": COMMIT, "down_id": int(downtime_id)},
    )
    assert SUCCESS in resp.text
...
def services_in_maintenance():
    resp = requests.get(
        env.nagios + "extinfo.cgi?type=6", auth=env.nagios_auth
    )
    ...
    for tr in nodes[1]:
        ...
        services.append(
            {"host": host, "service": service, "reason": reason, "id": down_id}
        )
    return services
